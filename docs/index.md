...
first `print("Hello, world!")` to production-grade, master-level engineering
— with runnable code snippets in every module and a hands-on project at the
end of each level.
...
- Every topic page has runnable code snippets — paste them into a Swift
  Playground, run with `swift` on the command line, or drop them into Xcode.
